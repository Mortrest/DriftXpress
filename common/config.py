...
class MultiResDriftConfig:
    ...
    # Multi-res encoder
    dataset: str = "cifar10"
    encoder: str = "dinov3"  # dinov2-multires, convnextv2, mocov2
    pool_size: int = 4  # spatial pool target per stage
    more_features: bool = False
    drift_global_query_gather: bool = True
    nystrom_landmarks_per_class: int = 500
    nystrom_total_landmarks: int | None = None
    nystrom_landmark_strategy: str = "kmeans_per_class"
    nystrom_landmark_class: int | None = None
    nystrom_shard_by_class: bool = False
    nystrom_classes_per_shard: int = 1
    nystrom_shard_cpu_offload: bool = False
    nystrom_ridge: float = 1e-4
    nystrom_landmark_seed: int = 0
    nystrom_kmeans_iters: int = 20
    nystrom_facility_candidate_ratio: int = 4
    nystrom_facility_eval_ratio: int = 8
    nystrom_density_knn_k: int = 32
    nystrom_density_reference_size: int = 1024
    nystrom_repulsion: str = "exact"
    nystrom_global_query_gather: bool = False
    nystrom_folded_exact_attraction: bool = False
    # Logging
    log_every: int = 100
