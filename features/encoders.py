...
def _extract_spatial_features(feat_map, pool_size=4, more_features=False):
    ...
    Returns:
        ...
        Default mode contains: per-location vectors, global mean, global std.
        more_features mode adds pooled global stats and patch mean/std features.
    """
    ...
    per_loc = feat_map.reshape(B, C, H * W).permute(0, 2, 1)
    result.append((per_loc, C))

    g_mean = feat_map.mean(dim=[2, 3]).unsqueeze(1)
    result.append((g_mean, C))

    g_std = feat_map.reshape(B, C, -1).std(dim=2, unbiased=False).unsqueeze(1)
    result.append((g_std, C))

    for k in (2, 4):
        if H >= k and W >= k:
            patch_mean = F.avg_pool2d(feat_map, kernel_size=k, stride=k)
            patch_sq_mean = F.avg_pool2d(feat_map.pow(2), kernel_size=k, stride=k)
            patch_var = (patch_sq_mean - patch_mean.pow(2)).clamp(min=1e-8)
            patch_std = torch.sqrt(patch_var)

            pm = patch_mean.reshape(B, C, -1).permute(0, 2, 1)
            ps = patch_std.reshape(B, C, -1).permute(0, 2, 1)
            result.append((pm, C))
            result.append((ps, C))
...
def _append_input_x2_feature(x, groups):
    """Append encoder-input mean(x^2) per channel as [B, 1, C0]."""
    x2 = x.pow(2).mean(dim=[2, 3]).unsqueeze(1)
    groups.append((x2, x.shape[1]))
...
def _finalize_feature_groups(feature_groups, more_features=False):
    """Keep feature groups separate in more_features mode."""
    if more_features:
        return feature_groups
    return _group_by_channel_dim(feature_groups)
...
class DINOv2MultiResEncoder(nn.Module):
    ...
    def forward(self, x):
        more_features = getattr(self, "more_features", False)
        x = (x + 1) / 2
        ...
        all_groups = []
        if more_features:
            _append_input_x2_feature(x, all_groups)
        for feat_map in intermediate:
            groups = _extract_spatial_features(feat_map, self.pool_size, more_features=more_features)
            all_groups.extend(groups)

        return _finalize_feature_groups(all_groups, more_features=more_features)
...
class MoCoV2Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for stage in [self.layer1, self.layer2, self.layer3, self.layer4]:
            h = stage(h)
            groups = _extract_spatial_features(h, self.pool_size, more_features=more_features)
            all_groups.extend(groups)
...
class CLIPMultiResEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        all_groups = []
        if more_features:
            _append_input_x2_feature(x_input, all_groups)
...
def build_encoder(name, pool_size=4, input_size=112, more_features=False):
    ...
    if name == "dinov2-multires":
        encoder = DINOv2MultiResEncoder(pool_size=pool_size, input_size=input_size)
    elif name == "convnextv2":
        encoder = ConvNeXtV2Encoder(pool_size=pool_size, input_size=input_size)
    elif name == "mocov2":
        encoder = MoCoV2Encoder(pool_size=pool_size, input_size=input_size)
    elif name == "dinov3":
        encoder = DINOv3MultiResEncoder(pool_size=pool_size, input_size=input_size)
    elif name == "dinov3-l":
        sz = input_size if input_size >= 224 else 224
        encoder = DINOv3LargeMultiResEncoder(pool_size=pool_size, input_size=sz)
    elif name == "dinov3-h+":
        sz = input_size if input_size >= 224 else 224
        encoder = DINOv3HugePlusMultiResEncoder(pool_size=pool_size, input_size=sz)
    elif name == "aimv2-l":
        sz = input_size if input_size >= 224 else 224
        encoder = AIMv2MultiResEncoder(pool_size=pool_size, input_size=sz, variant="large")
    elif name == "aimv2-h":
        sz = input_size if input_size >= 224 else 224
        encoder = AIMv2MultiResEncoder(pool_size=pool_size, input_size=sz, variant="huge")
    elif name == "radio":
        sz = input_size if input_size >= 224 else 224
        encoder = RADIOMultiResEncoder(pool_size=pool_size, input_size=sz)
    elif name == "eva02":
        encoder = EVA02MultiResEncoder(pool_size=pool_size, input_size=224)
    elif name == "siglip2":
        sz = input_size if input_size >= 224 else 224
        encoder = SigLIP2MultiResEncoder(pool_size=pool_size, input_size=sz)
    elif name == "clip":
        sz = input_size if input_size >= 224 else 224
        encoder = CLIPMultiResEncoder(pool_size=pool_size, input_size=sz)
    else:
        raise ValueError(
    ...
    encoder.more_features = more_features
    return encoder
